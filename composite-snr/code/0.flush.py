...
# 只保留需要的列数
df_list = df_final[["normalized_name","l","b",'RA','Dec','size_radio',"size_coarse (arcmin)",'distance_min (kpc)','distance_max (kpc)',"age_min (yr)","s"]]
...
output_publish = "../cat/publish.csv"
df_list.to_csv(output_publish, index=False)
print(f"合并完成，新文件已保存为 {output_publish}")
print(f"共有{df_final.shape[0]}个源")
print(f"其中{df_final['pulsar-CCO'].count()}包含脉冲星")
# %%
